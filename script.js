...
function generateDetailsReport() {
  ...
  const roleCounts = ["admin", "lawyer", "assistant", "client"].map(role => {
    const count = users.filter(u => u.role === role).length;
    return `<p>${titleCase(role)}s: <strong>${count}</strong></p>`;
  }).join("");

  const activeCases = cases.filter(c => ["In Progress", "Review", "open"].includes(c.status)).length;
  const settledCases = cases.filter(c => ["Settled", "closed", "Closed"].includes(c.status)).length;
  const pendingCases = cases.length - activeCases - settledCases;
  ...
  elements.reportOutputText.innerHTML = `
    <div class="report-block">
      ...
      <p>Active Accounts: <strong style="color:#059669;">${activeCount}</strong></p>
      <p>Restricted Accounts: <strong style="color:#dc2626;">${blockedCount}</strong></p>
    </div>
    <div class="report-block">
      <h5>User Roles Breakdown</h5>
      ${roleCounts}
    </div>
    <div class="report-block">
      <h5>Case Activity</h5>
      <p>Total Cases: <strong>${cases.length}</strong></p>
      <p>Active / In Progress: <strong style="color:#1d4ed8;">${activeCases}</strong></p>
      <p>Settled / Resolved: <strong style="color:#059669;">${settledCases}</strong></p>
      <p>Other / Pending Review: <strong>${pendingCases}</strong></p>
    </div>
    <div class="report-block">
      <h5>Appointments</h5>
      <p>Total Appointments: <strong>${appointments.length}</strong></p>
      <p>Confirmed: <strong style="color:#059669;">${confirmedAppts}</strong></p>
      <p>Pending: <strong style="color:#d97706;">${pendingAppts}</strong></p>
    </div>
    <p class="report-meta">Generated by Admin · ${new Date().toLocaleString()}</p>
  `;
