    ...
    if (bookingForm) {
      bookingForm.addEventListener("submit", async (e) => {
        ...
        const newAppt = {
          id: `A-${Date.now()}`,
          clientId: user.id,
          lawyerId: "u-lawyer", // Default placeholder
          assistantId: null, // Left null so assistants in this category can claim it
          date: date,
          time: time,
          type: type,
          status: "Pending",
        ...
        // Notify Specialty Group
        addNotification("u-assistant-group", "New Specialty Request", `${user.name} requested a ${type} consultation. Available for claim.`);
