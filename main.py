...
# Conversation states for drafting documents
ASK_NAME, ASK_LOCATION, ASK_OFFENSE_DETAILS = range(3)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome_msg = (
        ...
        "/draft_document - Create a legal draft document\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        ...
        "8. <b>Draft Document:</b> Use /draft_document to create a legal draft.\n"
...
async def start_drafting(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if lang == 'Bangla':
        await update.message.reply_text("আসুন একটি আইনি ড্রাফট তৈরি করি। আপনার পুরো নাম কি?")
    else:
        await update.message.reply_text("Let's draft a legal document. What is your full name?")
    return ASK_NAME

async def receive_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Receive name and ask for location."""
    user_id = update.effective_user.id
    lang = user_prefs.get(user_id, {}).get('lang', 'English')
    
    context.user_data['draft_name'] = update.message.text
    if lang == 'Bangla':
        await update.message.reply_text("বুঝতে পেরেছি। ঘটনাটি কোথায় ঘটেছিল বা সংশ্লিষ্ট স্থানটি কোথায়?")
    else:
        await update.message.reply_text("Got it. Where did the incident take place or what is the relevant location?")
    return ASK_LOCATION

async def receive_location(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Receive location and ask for offense details."""
    user_id = update.effective_user.id
    lang = user_prefs.get(user_id, {}).get('lang', 'English')
    
    context.user_data['draft_location'] = update.message.text
    if lang == 'Bangla':
        await update.message.reply_text("অনুগ্রহ করে অপরাধের বিবরণ সংক্ষেপে দিন।")
    else:
        await update.message.reply_text("Please describe the offense details briefly.")
    return ASK_OFFENSE_DETAILS

async def receive_offense_details(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Receive offense details, generate the document, and send it."""
    user_id = update.effective_user.id
    lang = user_prefs.get(user_id, {}).get('lang', 'English')
    
    context.user_data['draft_offense'] = update.message.text
    
    if lang == 'Bangla':
        await update.message.reply_text("ধন্যবাদ। আপনার ডকুমেন্ট তৈরি করা হচ্ছে...")
    else:
        await update.message.reply_text("Thank you. Generating your document now...")
    
    name = context.user_data.get('draft_name', 'Unknown')
    location = context.user_data.get('draft_location', 'Unknown')
    offense = context.user_data.get('draft_offense', 'Unknown')
    ...
    pdf.set_font("Arial", size=12)
    # Using encode/decode to avoid character encoding issues with FPDF default latin-1
    # or simple text
    pdf.cell(200, 10, txt=f"Complainant / Name: {name}", ln=True, align='L')
    pdf.cell(200, 10, txt=f"Location of Incident: {location}", ln=True, align='L')
    pdf.multi_cell(0, 10, txt=f"Offense Details:\n{offense}")
    ...
    try:
        with open(temp_path, 'rb') as doc_file:
            caption = "Here is your formatted legal document in PDF format."
            if lang == 'Bangla':
               caption = "এখানে আপনার ফরম্যাট করা আইনি ডকুমেন্ট পিডিএফ আকারে দেওয়া হলো।"
            await update.message.reply_document(document=doc_file, filename=f"Legal_Draft_{name.replace(' ', '_')}.pdf", caption=caption)
    finally:
        os.remove(temp_path)
        
    # Clear user data for drafting
    context.user_data.pop('draft_name', None)
    context.user_data.pop('draft_location', None)
    context.user_data.pop('draft_offense', None)
        
    return ConversationHandler.END

async def cancel_drafting(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Cancel the document drafting."""
    user_id = update.effective_user.id
    lang = user_prefs.get(user_id, {}).get('lang', 'English')
    
    if lang == 'Bangla':
        await update.message.reply_text("ডকুমেন্ট ড্রাফটিং বাতিল করা হয়েছে।")
    else:
...
def main() -> None:
    ...
    # Document Drafting Conversation Handler
    draft_conv_handler = ConversationHandler(
        entry_points=[CommandHandler('draft_document', start_drafting)],
        states={
            ASK_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_name)],
            ASK_LOCATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_location)],
            ASK_OFFENSE_DETAILS: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_offense_details)]
        },
        fallbacks=[CommandHandler('cancel', cancel_drafting)]
    )
    application.add_handler(draft_conv_handler)
