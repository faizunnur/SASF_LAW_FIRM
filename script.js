...
const roleModules = {
  guest: ["profileSection", "caseSection", "appointmentSection", "scheduleSection", "documentSection", "notificationSection", "userManagementSection"],
  ...
  admin: ["profileSection", "caseSection", "appointmentSection", "scheduleSection", "documentSection", "notificationSection", "userManagementSection"]
};
...
function renderModules() {
  ...
  // Default to User Management for Admin, otherwise the first available module
  const defaultModule = user?.role === "admin" ? "userManagementSection" : (visibleModuleIds[0] || "profileSection");
...
function renderUserManagementForm() {
  ...
  elements.userManagementForm.innerHTML = `
    <label><span>Select User</span><select name="userId" required>${selectableUsers.map((item) => `<option value="${item.id}">${item.name} (${titleCase(item.role)})</option>`).join("")}</select></label>
    <label><span>Assign/Update Role</span><select name="role" required>${["client", "lawyer", "assistant"].map((item) => `<option value="${item}">${titleCase(item)}</option>`).join("")}</select></label>
...
function generateReport() {
  ...
  const summary = `System Overview: ${totalUsers} total users (${activeUsers} active). 
    Database contains ${appState.data.cases.length} cases and ${appState.data.appointments.length} scheduled appointments.`;
  elements.reportOutputText.textContent = summary;
  showToast("Report generated for the system.");
}
